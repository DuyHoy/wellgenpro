...
$.extend(Microfiche.prototype, {
  ...
  // We create our film element, which weâ€™ll slide back and forth in the screen.
  // Before appending any extra elements, we detach the existing children,
  // append them to film, and tell them to float so theyâ€™ll (hopefully) lay-out
  ...
  // This slightly strange process tries to ensure we donâ€™t get any wrapping
  ...
  createButtons: function() {
    $('<button>')
    .addClass('microfiche-button microfiche-prev-button glyphicon glyphicon-chevron-left')
    .attr('rel', 'prev')
    .data('action', 'prev')
    .data('arguments', [])
    ...
    $('<button>')
    .addClass('microfiche-button microfiche-next-button glyphicon glyphicon-chevron-right')
    .attr('rel', 'next')
    .data('action', 'next')
    .data('arguments', [])
  ...
  //
  // If weâ€™re not already in drag-mode, we check to see if the horizontal
  ...
  // to be a drag, weâ€™ll deduce the new target value for x, ensure Microfiche
  ...
  // Handle what happens in cyclic mode if weâ€™ve slipped off at either end.
  ...
  // Slide to the previous screenâ€™s-worth of slides.
  prev: function() {
    this.slideByPages(-1);
  },

  // Slide to the next screenâ€™s-worth of slides.
  next: function() {
    this.slideByPages(1);
  },
